...
type
  ...
  public
    ...
    function MsgToString(const _Msg: TMsg): string; overload;
  end;
...
type
  TMessageStringItemList = class(TList);
...
begin
  ...
  for i := 0 to List.Count - 1 do begin
    if TMessageStringItem(List.Items[i]).FWmMsg = _WmMsg then begin
      Item := TMessageStringItem(List.Items[i]);
      Break;
    end;
  end;
  if Item <> Nil then
    ...
    Result := '$' + IntToHex(_WmMsg, 8) + ' (' + IntToStr(_WmMsg) + ')';
end;

function TWmMessageToString.MsgToString(const _Msg: TMessage): string;
begin
  Result := Format('%0:s (WParam: %1:x (%1:d), LParam: %2:x (%2:d))', [MsgToString(_Msg.Msg), _Msg.wParam, _Msg.LParam]);
end;

function TWmMessageToString.MsgToString(const _Msg: TMsg): string;
var
  Msg: TMessage;
begin
  Msg.Msg := _Msg.message;
  Msg.WParam := _Msg.wParam;
  Msg.LParam := _Msg.lParam;
  Result := MsgToString(Msg);
end;
...
begin
  ...
  for i := 0 to List.Count - 1 do begin
    if TMessageStringItem(List.Items[i]).FWmMsg = _WmMsg then begin
      Item := TMessageStringItem((FList as TMessageStringItemList).Items[i]);
      Break;
    end;
  end;
  if Item <> Nil then
